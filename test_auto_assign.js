/**
 * Test script: verifies BookNowAutoAssignService logic against live DB
 * Run: node test_auto_assign.js
 */
...
async function findBestPartner(task, profiles) {
  ...
  const postedArea = task.location?.taskArea || task.location?.locality || task.location?.city || '';
  ...
  console.log(`\n📋 Task: "${task.title}" | Category: ${parentCategory} | Area: ${postedArea} | Time: ${task.scheduledTimeStart || task.timeSlot}`);
  console.log(`📌 Search order: ${orderedWorkAreas.map(w => `${w.area}(${w.distKm.toFixed(1)}km)`).join(' → ')}`);
  ...
  for (const wa of orderedWorkAreas) {
    ...
    const distLabel = best => best.distKm !== null ? `${best.distKm.toFixed(2)} km` : 'Location Not Set';

    if (areaPartners.length > 0) {
      console.log(`\n✅ Best partner found in "${wa.area}" (${wa.distKm.toFixed(2)} km from task):`);
      console.log(`   → Name: ${areaPartners[0].name}`);
      console.log(`   → UID:  ${areaPartners[0].uid}`);
      console.log(`   → Distance: ${distLabel(areaPartners[0])}`);
      if (areaPartners.length > 1) {
        console.log(`   (${areaPartners.length - 1} other eligible partners in this area were not selected)`);
      }
      return areaPartners[0];
    } else {
      console.log(`   ⚠️  No eligible partners in "${wa.area}" — checking next area...`);
    }
  }

  console.log(`\n❌ No eligible partner found in any work area within 5km. Task requires manual ops assignment.`);
  return null;
}

async function run() {
  await mongoose.connect(MONGO_URI, { dbName: 'extrahand' });
  const db = mongoose.connection.db;

  const task = await db.collection('tasks').findOne({ _id: new mongoose.Types.ObjectId('6a7c056a4aea573e5bad61bb') });
  if (!task) { console.error('Task not found'); process.exit(1); }

  const profiles = await db.collection('profiles').find({ isActive: true, 'partnerProfile.status': 'approved' }).toArray();
  console.log(`Total approved profiles: ${profiles.length}`);

  await findBestPartner(task, profiles);
  process.exit(0);
}
