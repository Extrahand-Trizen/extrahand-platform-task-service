...
export class TaskService {
  ...
  static async updateTaskStatus(
    ...
    // Creators can update to any status except 'started' (new tasks can't jump straight to started)
    // They CAN update to 'in_progress' when rejecting completion (requesting changes)
    if (isCreator && !isAssignedPerformer) {
      // Only restrict jumping to 'started' status initially
      // Allow 'in_progress' so poster can request changes by reverting from 'review'
      if (status === 'started' && task.status !== 'started') {
        throw new ForbiddenError('Only assigned performers can start a task');
      }
    }
