...
/**
 * M1 product rule — single-visit Hourly checkout.
 * Not a permanent Booking invariant (packs/plans later).
 */
export function assertHourlySingleVisitCheckout(params: {
  ...
  if (params.lines.length !== 1) {
    throw new BadRequestError('Hourly Helper allows exactly one service line per booking');
  }
