...
export class CascadeDeleteController {
  ...
  static async getAccountDeletionPreview(req: Request, res: Response): Promise<void> {
    ...
    const deletionScope = String(req.headers['x-deletion-scope'] || 'full');
    ...
    logger.info(`🔎 Account deletion preview for user: ${uid}`, { profileId, deletionScope });



    const result = await CascadeDeleteService.getAccountDeletionPreview(profileId, deletionScope);
  ...
  static async deleteAccountEligibleData(req: Request, res: Response): Promise<void> {
    ...
    logger.info(`🗑️ Account-deletion eligible delete for user: ${uid}`, {
    ...
    const result = await CascadeDeleteService.deleteAccountEligibleData(
  ...
  static async getActiveDeletionBlockers(req: Request, res: Response): Promise<void> {
    ...
    logger.info(`🔎 Active deletion blockers check for user: ${uid}`, {
    ...
    const result = await CascadeDeleteService.getActiveDeletionBlockers(
